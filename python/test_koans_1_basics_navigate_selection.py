...
class TestKoans1BasicsNavigateSelection(TestCase):
    ...
    def test_koan01_navigate_to_end_block_and_find_the_key_to_open_the_door(self):
        ...
        # TIPS : on laptop keyboard, you can navigate to end of file by using / Ctrl+A, →
        self.assertEqual(
    ...
    def test_koan02_deletion_to_next_word(self):
        ...
        self.assertTrue("    " in "Delete spaces after me:to much spaces kill spaces")

    def test_koan03_deletion_to_word_start_word(self):
        # Delete MouseForEver by using delete to Word Start Ctrl+Backspace
        self.assertTrue("MouseForEver" in "delete the last straw!")

    def test_koan04_delete_the_failing_line(self):
        delete_unused_line = "This test should pass"
        delete_unused_line += "*** delete me by using Ctrl + Shift + K ****";
        self.assertEqual(delete_unused_line, "This test should pass")

    def test_koan05_extend_selection(self):
        # by using Shift + Alt + right (Alt + left to unselect)
        self.assertEqual("***** SELECT ME WITH EXTEND SELECTION*****", "selected")

    def test_koan06_extend_selection_code_scope(self):
        # extend selection depends on scope, by using Shift + Alt + right select only what needed in line below
        # you have to put cursor on test and use the shortcut, then copy the text : "ModuleA().test()"
        self.assertEqual(ModuleA().test(), "youpii")
        # then paste it in the following assertion
    ...
    def test_koan08_select_next_occurrence_then_edit_multi_lines(self):
        # by using Ctrl + D (press Esc to exit multi caret edition)
    ...
    def test_koan09_select_all_occurrences_then_edit_multi_carets(self):
        # 1. select TO CHANGE OCCURRENCE text
        # 2. Ctrl + F to start search
        # 3. Alt + Enter to select all occurrences
        # 4. replace TO CHANGE OCCURRENCE by allOccurences
        message = "Lorem ipsum TO_CHANGE_OCCURRENCE sit amet, consectetur adipiscing elit.\n"
        message += "Nam TO_CHANGE_OCCURRENCE dignissim sem at posuere. Donec sollicitudin est ac lorem eleife,\n"
        message += "eget aliquam velit consectetur. Aliquam id TO_CHANGE_OCCURRENCE sit amet nisl suscipit iaculis\n"
        message += "nec et magna. TO_CHANGE_OCCURRENCE in felis ut lacus gravida faucibus. Suspendisse dictum\n"
        message += "at sem ac semper. TO_CHANGE_OCCURRENCE venenatis placerat ipsum vel bibendum. Donec et\n"
        message += "diam nibh.\n"
